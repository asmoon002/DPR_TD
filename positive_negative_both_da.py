...
def calculate_weights(doc_tokens, df_counts):
    weights=[]
    for doc_token in doc_tokens:
        weights.append({'token':doc_token, 'weight':(doc_tokens.count(doc_token)*df_counts[doc_token])})
    ...
    # Scaler to make the median value 0.1
        scale_factor = 0.2
...
def process_chunk(data_chunk, tokenizer):
    ...
    for doc_index, doc_tokens in enumerate(tokenized_documents):
        weights = calculate_weights(doc_tokens, df_counts)
...
# negative 프로포즈드 메소드 적용
def process_chunk_negative(data_chunk, tokenizer):
    documents = []
    negative_ctx_indices = []

    for item in data_chunk:
        for ctx_index, ctx in enumerate(item['negative_ctxs']):
            documents.append([ctx['text']])
            negative_ctx_indices.append((item, ctx_index))
    ...
    for i, reconstructed_sentence in enumerate(save_result):
        item, ctx_index = negative_ctx_indices[i]
        item['negative_ctxs'][ctx_index]['text'] = reconstructed_sentence
...
def main():
    try:
        ...
        for j, chunk in enumerate(chunks):
            ...
            save_result_pos, modified_data = process_chunk(chunk, tokenizer)
            save_result_neg, modified_data = process_chunk_negative(modified_data, tokenizer)
            ...
            #경로 수정
            save_json(modified_data, f'save_path/pos_neg_both{j+1}.json')
