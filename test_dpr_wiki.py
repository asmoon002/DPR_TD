...
if __name__ == '__main__':
    ...
    #수정
    parser.add_argument("--wikipedia_path",default="파일 경로/WikiQA_Docs_split.tsv")
    parser.add_argument("--nq_test_file",default="파일 경로/WikiQA_test_preprocessed.tsv")
    #parser.add_argument("--encoding_batch_size",type=int,default=32)
    #parser.add_argument("--num_shards",type=int,default=8)
    parser.add_argument("--encoding_batch_size",type=int,default=4)
    ...
    def preprocess_answers(answer_str):
    
        if not answer_str.startswith("[") or not answer_str.endswith("]"):
            return "[]"  # Return an empty list as string if format is invalid

    # Return the string as-is for eval
        return answer_str
    ...
    with open(args.nq_test_file) as f:
        ...
        for row in reader:
            ...
            answers.append(eval(preprocess_answers(row[answers_col])))
